page 50077 "Inventory Analysis By Seg SNS"
{
            ...
            cuegroup(control3)
            {
                ...
                field(Inv181to360Blank; Inv181to360Blank)
                {
                    ...
                    begin
                        Clear(ITEMDetails);
                        ItemCatRec.Reset();
                        ItemCatRec.SetFilter("Inv Below 181-360 Days Blank SNS", '<>%1', 0);
                        ITEMDetails.SetTableView(ItemCatRec);
    ...
    begin
        ...
        if ILE.FindSet() then
            repeat
                if ItemRec.Get(ILE."Item No.") and (ItemRec."Prod Segment" = ItemRec."Prod Segment"::CSG) and (ItemRec."Item Brand" <> '') then begin
        ...
        if ILE.FindSet() then
            repeat
                if ItemRec.Get(ILE."Item No.") and (ItemRec."Prod Segment" = ItemRec."Prod Segment"::ISG) and (ItemRec."Item Brand" <> '') then begin
                    ILE.CalcFields("Cost Amount (Actual)");
                    if ILE.Quantity <> 0 then
        ...
        if ILE.FindSet() then
            repeat
                if ItemRec.Get(ILE."Item No.") and (ItemRec."Prod Segment" <> ItemRec."Prod Segment"::CSG) and (ItemRec."Prod Segment" <> ItemRec."Prod Segment"::ISG) and (ItemRec."Item Brand" <> '') then begin
                    ILE.CalcFields("Cost Amount (Actual)");
                    if ILE.Quantity <> 0 then
