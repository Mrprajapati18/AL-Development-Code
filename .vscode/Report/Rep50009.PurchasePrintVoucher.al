report 50009 "Purchase Print Voucher"
{
    Caption = 'PurchInvPrintVoucherNew';
    UsageCategory = ReportsAndAnalysis;
    ApplicationArea =  All;
    DefaultLayout = RDLC;
    RDLCLayout = 'Layouts\PurchInvPrintVoucherNew.rdl';
    ...
    dataset
    {
        dataitem("G/L Entry"; "G/L Entry")
        {
            ...
            column(purchinvcomment; purchinvcomment)
            {
            }
            ...
            begin
                ...
                ChequeNo:='';
                ChequeDate:=0D;
                if("Source No." <> '') and ("Source Type" = "Source Type"::"Bank Account")then if BankAccLedgEntry.Get("Entry No.")then begin
                ...
                // Clear(purchinvcomment);
                // GLEntry.Reset();
                // purchcommentline.Reset();
                // GLEntry.SetRange("Document No.", purchinvhead."No.");
                // if GLEntry.FindFirst() then begin
                //     purchcommentline.SetRange("No.", GLEntry."Document No.");
                //     if purchcommentline.FindFirst() then begin
                //         purchinvcomment += purchcommentline.Comment;
                //     end;
                // end;
                Clear(purchinvcomment);
                purchinvhead.Reset();
                purchinvhead.SetRange("No.", "G/L Entry"."Document No.");
                if purchinvhead.FindFirst()then begin
                    purchcommentline.Reset();
                    purchcommentline.SetRange("No.", purchinvhead."No.");
                    if purchcommentline.FindFirst()then repeat if purchinvcomment = '' then purchinvcomment:=purchcommentline.Comment
                            else
                                purchinvcomment+=' , ' + purchcommentline.Comment;
    ...
    procedure FindGLAccName("Source Type": Enum "Gen. Journal Source Type"; "Entry No.": Integer; "Source No.": Code[20]; "G/L Account No.": Code[20]): Text[100]var
        ...
        AccName: Text[100];
    begin
        case "Source Type" of "Source Type"::Vendor: if VendorLedgerEntry.Get("Entry No.")then if IsGSTDocument(VendorLedgerEntry."Document Type", VendorLedgerEntry."Document No.")then begin
                    if Vendor.Get("Source No.")then AccName:=Vendor.Name
                    else
                    begin
        ...
        "Source Type"::Customer: if CustLedgerEntry.Get("Entry No.")then if IsGSTDocument(CustLedgerEntry."Document Type", CustLedgerEntry."Document No.")then begin
                    if Customer.Get("Source No.")then AccName:=Customer.Name
                    else
                    begin
                        GLAccount.Get("G/L Account No.");
                        AccName:=GLAccount.Name;
        ...
        "Source Type"::"Bank Account": if BankAccountLedgerEntry.Get("Entry No.")then begin
    ...
    local procedure IsGSTDocument(DocumentType: Enum "Gen. Journal Document Type"; DocumentNo: Code[20]): Boolean var
        DetailedGSTLedgerEntry: Record "Detailed GST Ledger Entry";
    begin
        DetailedGSTLedgerEntry.SetRange("Document No.", DocumentNo);
        if DetailedGSTLedgerEntry.FindFirst()then exit(true);
        ...
        if not DetailedGSTLedgerEntry.IsEmpty then exit(true);
        exit(false);
    end;
    local procedure GetApplicationDocType(DocumentType: Enum "Gen. Journal Document Type"): Enum "Application Doc Type" var
        ApplicationDocType: Enum "Application Doc Type";
    begin
        case DocumentType of DocumentType::Invoice: exit(ApplicationDocType::Invoice);
        DocumentType::"Credit Memo": exit(ApplicationDocType::"Credit Memo");
        DocumentType::"Finance Charge Memo": exit(ApplicationDocType::"Finance Charge Memo");
        DocumentType::Refund: exit(ApplicationDocType::Refund);
        DocumentType::Payment: exit(ApplicationDocType::Payment);
        DocumentType::" ": exit(ApplicationDocType::" ")end;
    end;
}
